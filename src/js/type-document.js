...
const validarErrores = function (serverError = null, editar = false, limpiar = false) {
  ...
  const $inputTpDoc = editar ? $tpDocumentoModal : $tpDocumento;
  const $inputDetalleDoc = editar ? $detalleDocumentoModal : $detalleDocumento;
  const $listErrors = editar ? $liErrorsTpDocumentModal : $liErrors;
...
d.addEventListener('DOMContentLoaded', async e => {
  $tpDocumento = d.getElementById('tp-documento');
  $detalleDocumento = d.getElementById('detalle-documento');
  $liErrors = d.getElementById('errors');


  // INPUTS DE LA MODAL
  $tpDocumentoModal = d.querySelector('#editTypeDocumenModal #tp-documento');
  $detalleDocumentoModal = d.querySelector('#editTypeDocumenModal #detalle-documento');
  $idTpDocumentModal = d.querySelector('#editTypeDocumenModal #id-type-document');
  $liErrorsTpDocumentModal = d.querySelector('#editTypeDocumenModal #errors');
...
d.addEventListener('submit', async e => {
  if (e.target.matches('#form-type-documents')) {
    ...
    const response = await fetchRequest(onErrorResponse, onErrorCatch, `${API_URL}/type-document/create-type-document`, 'POST', {
      name: $tpDocumento.value,
      description: $detalleDocumento.value,
    });
...
d.addEventListener('click', async e => {
  ...
  if (e.target.matches('#btn-edit-product, #btn-edit-product > *')) {
    ...
    if (response) {
      ...
      $tpDocumentoModal.value = response.data.name;
      $detalleDocumentoModal.value = response.data.description;
  ...
  if (e.target.matches('#btn-guardar-cambios')) {
    ...
    console.log({
      name: $tpDocumentoModal.value,
      description: $detalleDocumentoModal.value,
    });
